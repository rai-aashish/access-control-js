...
describe("Policy Contexts (ABAC)", () => {
	...
	it("should merge default context with check context", () => {
		const policy = definePolicy<AppConfig>()
			.allow("posts", ["create"], {
				contexts: [{ role: "admin", churchId: "123" }],
			})
			.build();

		// Initialize with a default context (e.g. tenant ID)
		const { can } = getAccessControl(policy, {
			defaultContext: { churchId: "123" },
		});
		...
		expect(can("posts", "create", { role: "admin", churchId: "456" })).toBe(
			false,
		);
	});
	...
	it("should handle multiple allowed contexts (OR logic)", () => {
		const policy = definePolicy<AppConfig>()
			.allow("posts", ["update"], {
				contexts: [{ role: "owner" }, { role: "editor", status: "draft" }],
			})
			.build();
		...
		expect(can("posts", "update", { role: "owner", status: "published" })).toBe(
			true,
		);

		// Schema 2: editor can only update if status is draft
		expect(can("posts", "update", { role: "editor", status: "draft" })).toBe(
			true,
		);
		expect(
			can("posts", "update", { role: "editor", status: "published" }),
		).toBe(false);
	});
	...
	it("should work with canAll and mixed contexts", () => {
		...
		expect(canAll("billing", ["view", "manage"], { role: "admin" })).toBe(
			false,
		);
	...
	it("should use default context in canAll checks", () => {
		const policy = definePolicy<AppConfig>()
			.allow("comments", ["delete"], {
				contexts: [{ role: "moderator", churchId: "123" }],
			})
			.build();
