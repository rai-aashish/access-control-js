...
describe("Production Scenarios", () => {
	describe("Scenario 1: RBAC + ABAC Hybrid (Department Head)", () => {
		...
		it("prevents managers from approving other departments (implicit deny)", () => {
			...
			expect(
				can("budget", "approve", { role: "manager", department: "sales" }),
			).toBe(false);
		});
		...
		it("prevents cross-department approval context mismatch", () => {
			...
			expect(
				can("budget", "approve", { role: "manager", department: "marketing" }),
			).toBe(true);
		});
	});
	...
	describe("Scenario 2: Deny Overrides (Blacklist)", () => {
		...
		const policy = definePolicy<AppConfig>()
			...
			.deny("document", ["read"], {
				contexts: [{ classification: "restricted" }],
			})
			.build();
		...
		it("allows user to read normal documents", () => {
			expect(
				can("document", "read", { role: "user", classification: "public" }),
			).toBe(true);
		});

		it("denies user reading restricted documents", () => {
			expect(
				can("document", "read", { role: "user", classification: "restricted" }),
			).toBe(false);
		});
		...
		it("denies access if only classification matches (must have Allow role too)", () => {
			...
			expect(
				can("document", "read", { role: "guest", classification: "public" }),
			).toBe(false);
		});
	});
	...
	describe("Scenario 4: Multi-Profile User (Dual Role)", () => {
		...
		const policy = definePolicy<AppConfig>()
			...
			.allow("post", ["publish"], {
				contexts: [{ role: "member", status: "vip" }],
			})
			.build();
	...
	describe("Scenario 5: Conflict Resolution (Ordering)", () => {
		...
		it("handles denyWins (default/standard)", () => {
			...
			expect(can("document", "delete", { role: "admin", unsafe: true })).toBe(
				false,
			);

			// Matches only Allow -> Allow
			expect(can("document", "delete", { role: "admin", unsafe: false })).toBe(
				true,
			);
		});
	...
	describe("Scenario 6: Bulk Checks (canThese)", () => {
		...
		it("returns an object with multiple permission results", () => {
			const results = canThese("post", ["read", "create", "update", "delete"], {
				role: "user",
			});
		...
		it("works with merged default context", () => {
			const { canThese: canTheseWithCtx } = getAccessControl(policy, {
				defaultContext: { role: "user" },
			});
...
describe("Scenario: ownership and tenancy from one policy", () => {
	// One policy serves every user — identity arrives in the context at check time,
	// so nothing here has to be rebuilt when the current user changes.
	const policy = definePolicy<AppConfig>()
		.allow("document", ["read"], {
			contexts: [{ "doc.tenantId": { $ref: "user.tenantId" } }],
		})
		.allow("document", ["write"], {
			contexts: [
				{
					"doc.tenantId": { $ref: "user.tenantId" },
					"doc.ownerId": { $ref: "user.id" },
				},
			],
		})
		.allow("document", ["write"], {
			contexts: [{ "user.roles": { $contains: "admin" } }],
		})
		.deny("document", ["write"], {
			contexts: [{ "doc.state": { $in: ["locked", "archived"] } }],
		})
		.build();

	const alice = { id: 1, tenantId: "acme", roles: ["editor"] };
	const admin = { id: 2, tenantId: "acme", roles: ["admin"] };

	it("reads within the tenant but not across it", () => {
		const { can } = getAccessControl<AppConfig>(policy);

		expect(
			can("document", "read", { user: alice, doc: { tenantId: "acme" } }),
		).toBe(true);
		expect(
			can("document", "read", { user: alice, doc: { tenantId: "globex" } }),
		).toBe(false);
	});

	it("writes own documents only", () => {
		const { can } = getAccessControl<AppConfig>(policy);
		const own = { tenantId: "acme", ownerId: 1, state: "draft" };
		const other = { tenantId: "acme", ownerId: 99, state: "draft" };

		expect(can("document", "write", { user: alice, doc: own })).toBe(true);
		expect(can("document", "write", { user: alice, doc: other })).toBe(false);
	});

	it("lets an admin write documents they do not own", () => {
		const { can } = getAccessControl<AppConfig>(policy);

		expect(
			can("document", "write", {
				user: admin,
				doc: { tenantId: "acme", ownerId: 99, state: "draft" },
			}),
		).toBe(true);
	});

	it("under the default strategy the lock holds against the admin but not the owner", () => {
		const { can } = getAccessControl<AppConfig>(policy);
		const locked = { tenantId: "acme", ownerId: 1, state: "locked" };

		// The admin allow and the deny are both one key, and a deny breaks the tie
		expect(can("document", "write", { user: admin, doc: locked })).toBe(false);
		// The ownership allow is two keys, so it outranks the one-key deny
		expect(can("document", "write", { user: alice, doc: locked })).toBe(true);
	});

	it("explicitDenyWins makes the lock hold for everyone", () => {
		const { can } = getAccessControl<AppConfig>(policy, {
			conflictResolution: "explicitDenyWins",
		});
		const locked = { tenantId: "acme", ownerId: 1, state: "locked" };

		expect(can("document", "write", { user: alice, doc: locked })).toBe(false);
		expect(can("document", "write", { user: admin, doc: locked })).toBe(false);
	});
});
