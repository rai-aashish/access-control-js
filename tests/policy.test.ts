...
describe("Policy indexing", () => {
	it("indexes the policy when the instance is built, so later in-place mutation is not seen", () => {
		...
		policy.length = 0; // bypasses the supported updatePolicy path

		// Statements are grouped by resource up front, so the instance keeps answering
		// from the policy it was handed
